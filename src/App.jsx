...
function VisionBoard({ session }) {
    ...
    return (
      <div style={nightStyle}>
        <div style={{ position: 'absolute', top: '16px', right: '16px', display: 'flex', gap: '8px' }}>
             <button onClick={() => setMode('morning')} style={{ border: '1px solid #333', padding: '8px 16px', borderRadius: '20px', fontSize: '12px', color: '#888', background: 'rgba(0,0,0,0.5)', cursor: 'pointer', backdropFilter: 'blur(4px)' }}>Morning ☀️</button>
             <button onClick={handleLogout} style={{ border: '1px solid #ef4444', padding: '8px', borderRadius: '50%', color: '#ef4444', background: 'rgba(0,0,0,0.5)', cursor: 'pointer' }}><LogOut size={14} /></button>
        </div>
        ...
        <div style={{ maxWidth: '400px', width: '100%', display: 'flex', flexDirection: 'column', gap: '30px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <button onClick={handleCapture} disabled={uploading} style={{ flex: 1, backgroundColor: uploading ? '#333' : 'white', color: 'black', fontWeight: 'bold', border: 'none', borderRadius: '16px', cursor: 'pointer', fontSize: '16px', boxShadow: '0 0 15px rgba(255,255,255,0.1)' }}>{uploading ? 'Syncing...' : 'Capture Vision'}</button>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={morningStyle}>
       <div style={{ position: 'absolute', top: '16px', right: '16px', display: 'flex', gap: '8px' }}>
          <button onClick={() => setMode('night')} style={{ border: '1px solid #cbd5e1', padding: '8px 16px', borderRadius: '20px', fontSize: '12px', color: '#64748b', background: 'rgba(255,255,255,0.5)', cursor: 'pointer', backdropFilter: 'blur(4px)' }}>Capture 🌙</button>
          <button onClick={handleLogout} style={{ border: '1px solid #ef4444', padding: '8px', borderRadius: '50%', color: '#ef4444', background: 'rgba(255,255,255,0.5)', cursor: 'pointer' }}><LogOut size={14} /></button>
       </div>
      ...
      <div style={{ maxWidth: '400px', width: '100%', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          {visibleThoughts.length === 0 && (<div style={{ padding: '40px 0', textAlign: 'center', color: '#94a3b8' }}><p style={{ margin: 0, fontSize: '16px' }}>{activeTab === 'targets' ? "All targets destroyed. Good work." : "The Vault is empty."}</p></div>)}
          {visibleThoughts.map((thought) => (
            <div key={thought.id} style={{ backgroundColor: thought.ignited ? 'rgba(240, 253, 244, 0.9)' : 'rgba(255, 255, 255, 0.8)', border: `1px solid ${thought.ignited ? '#bbf7d0' : '#e2e8f0'}`, borderRadius: '20px', overflow: 'hidden', paddingBottom: '16px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)', backdropFilter: 'blur(8px)' }}>
              {thought.image_url && (<div style={{ width: '100%', height: '260px', overflow: 'hidden' }}><img src={thought.image_url} alt="Vision" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /></div>)}
              <div style={{ padding: '0 24px', marginTop: '20px' }}>
                ...
                <p style={{ fontSize: '19px', fontWeight: '600', lineHeight: '1.5', margin: 0, color: thought.ignited ? '#94a3b8' : '#1e293b', textDecoration: thought.ignited ? 'line-through' : 'none' }}>"{thought.text}"</p>
