...
import { Moon, Sun, Archive, Target, Flame, LogOut, Lock, Mic, Video, Camera, X, Square, ListTodo, Quote as QuoteIcon, CheckSquare, Plus, Eye, RotateCcw, Trophy, ArrowLeft, Eraser, RefreshCcw, Trash2, ShieldCheck, AlertCircle, Edit3, Fingerprint, GripVertical } from 'lucide-react';
...
function VisionBoard({ session }) {
  ...
  const getSortedThoughts = () => { const relevant = thoughts.filter(t => viewingGoal === 'all' ? true : t.goal_id === viewingGoal.id); return relevant.sort((a, b) => Number(a.ignited) - Number(b.ignited)); };
  ...
  const handleReorder = (newOrder) => {
    // Combine the reordered active list with the static completed list
    setMissions([...newOrder, ...completedMissions]);
  };
